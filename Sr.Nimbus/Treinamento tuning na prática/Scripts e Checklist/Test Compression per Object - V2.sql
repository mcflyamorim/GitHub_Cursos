USE <DB You Want to test the compression>
...
SELECT schema_name + '.' + object_name AS Objeto,
       ...
       st.row_count AS 'Número linhas obj',
       data_compression_desc AS Compressao_Atual,
	   ROUND((CAST(Current_Size AS Numeric(18,2)) / 1024), 2) AS 'Tamanho Atual (MB)',
       ROUND((CAST(None_Size AS Numeric(18,2)) / 1024), 2) AS 'Estimado_Compressao_NONE(MB)',
       ROUND((CAST(Row_Size AS Numeric(18,2)) / 1024), 2) AS 'Estimado_Compressao_ROW(MB)',
       ROUND(CAST(Page_Size AS Numeric(18,2)) / 1024, 2) AS 'Estimado_Compressao_PAGE(MB)',
       index_fill_factor AS 'Fill Factor Atual',
       indexstats.avg_fragmentation_in_percent AS '% Fragmentação',
       ROUND((1 - (CAST(Row_Size AS Numeric(18,2)) / Current_Size)) * 100, 2) AS 'Ganho_Estimado_Linha(%)',
       ROUND((1 - (CAST(Page_Size AS Numeric(18,2)) / Current_Size)) * 100, 2) AS 'Ganho_Estimado_Pagina(%)',
       Tab1.Compressao_Recomendada,
       ISNULL(bp.CacheSizeMB,0) AS 'Espaco_Utilizado_BP(MB)',
       ISNULL(bp.FreeSpaceMB,0) AS 'Espaco_Livre_BP(MB)',
       CASE ixType 
