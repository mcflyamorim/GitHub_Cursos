...
*/
  ...
  BEGIN TRY
    ...
    BEGIN
      RAISERROR 30002 'Specified table do not exists';
    END

    IF INDEXPROPERTY(OBJECT_ID(@Table_Name), @Lookup_Index, 'IsClustered') <> 0
    BEGIN
      RAISERROR 30002 'Specified index must be a nonclustered index';
    END    
    ---------------------------------------------------------
    --------------- Drop temp tables ------------------------
    ---------------------------------------------------------
    ...
    IF ISNULL(@Col,'') = ''
    BEGIN
      RAISERROR 30002 'There is no "nonclustered/clustered" column';
    END
