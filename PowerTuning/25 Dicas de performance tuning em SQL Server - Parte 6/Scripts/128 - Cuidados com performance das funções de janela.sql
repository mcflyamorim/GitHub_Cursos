...
-- Query utilizando frame padrão (RANGE UNBOUNDED PRECEDING)
...
SELECT IDConta,
       ColData,
       ColValor,
       SUM(ColValor) OVER(PARTITION BY IDConta ORDER BY ColData ASC) AS RunningTotal
  FROM Tab_TesteSaldo
 WHERE IDConta = @i
...
-- E no SQL Query Stress? Como fica? ...


-- Para mais detalhes...
https://www.simple-talk.com/sql/learn-sql-server/window-functions-in-sql-server-part-3-questions-of-performance/
