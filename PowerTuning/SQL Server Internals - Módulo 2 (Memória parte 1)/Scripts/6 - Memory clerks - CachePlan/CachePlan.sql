/*
  ...
  Os seguintes caches são incluídos no cachestore: 
  1.	Compiled Plans (CP)
  1.1 - Queries dinâmicas e prepared = sql CP
...
-- Demo 1 - Visão geral e DMVs (dm_exec_cached_plans, dm_exec_query_stats e sys.syscacheobjects)
...
-- Demo 2 - Tá, mas eu preciso mesmo do plan cache? 
/*
  Tá, mas eu preciso mesmo do plan cache? 
*/
...
-- Abrir App 
-- D:\Fabiano\Trabalho\Sr.Nimbus\Cursos\SQL26 - SQL Server - Mastering the database engine (former Internals)\Slides\Módulo 02 - Memória parte 1\6 - Memory clerks - CachePlan\Loop Delphi
...
/*
  Ok, ainda não fui convencido de que isso realmente afeta performance.
*/
-- Limpar o PlanCache
DBCC FREEPROCCACHE
GO
...
/*
  O correto é utilizar parâmetros, mas faça isso corretamente.
*/

-- Demo 4 - Cuidados com parameterização na App e TF144

DBCC FREEPROCCACHE()
GO

-- Abrir app 
-- D:\Fabiano\Trabalho\FabricioLima\Cursos\SQL Server Internals - Módulo 2 (Memória parte 1)\Scripts\6 - Memory clerks - CachePlan\Cache Plans Test
/*
...
http://blogs.msdn.microsoft.com/sqlprogrammability/2007/01/13/6-0-best-programming-practices
...
-- Eita, forced param não funcionou... é, não faz pra tudo...
...
/*
  ...
  Guided plans podem ser utilizados para forçar plano na query
*/
...
-- Query não é auto parametrizada
SELECT *
  FROM OrdersBig
 WHERE OrderID < 100 AND ISNULL(CustomerID,0) = 100
GO
...
SELECT *
  FROM OrdersBig
 WHERE OrderID < 100 AND ISNULL(CustomerID,0) = 123123
GO
...
-- E se eu rodar esse demônio no SQLQueryStress? 
...
-- E se a gente tentar com forced pra evitar a recompilação? ...
-- Setar o banco como PARAMETERIZATION FORCED
ALTER DATABASE NorthWind SET PARAMETERIZATION FORCED WITH NO_WAIT
GO
...
-- Abrir app
-- D:\Fabiano\Trabalho\FabricioLima\Cursos\SQL Server Internals - Módulo 2 (Memória parte 1)\Scripts\6 - Memory clerks - CachePlan\sp_prepare vs direct exec
...
-- Conclusão
-- O tempo é o basicamente o mesmo, porém a visibilidade do código executado no profiler é péssima com sp_prepare...
-- Talvez em um ambiente com baixa latência de rede, o sp_prepare ajude, já que na 
-- teoria irá reduzir o tamanho do pacote enviado pela rede... Azure com serv no Japão...?...
...
AS
SELECT TOP 1000000 * FROM OrdersBig1
...
-- Um usuário diferente, também irá gerar um plano novo... Portanto o ideal seria rodar a query da app com o 
