USE master
GO
if exists (select * from sysdatabases where name='Desafio2')
BEGIN
  ALTER DATABASE Desafio2 SET SINGLE_USER WITH ROLLBACK IMMEDIATE
...
EXECUTE (N'CREATE DATABASE Desafio2
  ON PRIMARY (NAME = N''Desafio2'', FILENAME = N''' + @device_directory + N'Desafio2.mdf'')
  LOG ON (NAME = N''Desafio2_log'',  FILENAME = N''' + @device_directory + N'Desafio2.ldf'')')
GO

ALTER DATABASE Desafio2 SET RECOVERY SIMPLE
GO
...
ALTER TABLE OrdersBig ADD CONSTRAINT xpk_OrdersBig PRIMARY KEY(OrderID) WITH(FILLFACTOR = 20)
GO
...
GO
ALTER TABLE CustomersBig ADD CONSTRAINT xpk_CustomersBig PRIMARY KEY(CustomerID) WITH(FILLFACTOR = 20)
GO
